...
function Poster({ poster, title }: { poster?: string; title: string }) {
    ...
    const optimizedSmall = base ? `/img/optimized/${base}-card-small.webp` : null;
...
export default function PlayCardInline({ play }: { play: Play }) {
    return (
        <Link
            to={`/pieces/${play.slug}`}
            className="group glass flex transform-gpu gap-4 rounded-2xl p-3.5 transition-[background-color,box-shadow] duration-200 hover:bg-white/[0.06] hover:shadow-[0_6px_18px_rgba(212,160,83,0.08)] hover:ring-1 hover:ring-accent/20"
        >
